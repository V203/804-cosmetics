    ...
    return (
        <>
            <Flex alignItems="center" gap='2' p={2} zIndex={300} className="header">
                ...
                {<Text>Total  R {order}</Text>} <Spacer /> 
                
                {/* <img onClick={()=>isOpen} style={{ float: "right" }} width={60} height={60} src={"cart.svg"} /> */}
                        
                <Flex gap={2} alignItems="center" >
                <CartOverView />
                <Button alignContent={"center"} justifyContent={"center"} gap="2"   type={"button"} onClick={() => navigate("/Login")} outlineColor={themeStyles.color} colorScheme={themeStyles.btnColor} > <CgLogIn size={"xs"} /> Sign in</Button>
                <Button value={"Register"} type={"button"} onClick={() => navigate("/Register")} outlineColor={themeStyles.color} colorScheme={themeStyles.btnColor}> Register </Button>
                <Button colorScheme={"pink"} onClick={signOut}> <GoSignOut size={"xs"}/> Sign out</Button>
